...
try:
    ...
    pytest.skip(f"Memory System test dependencies not available: {e}", allow_module_level=True)
...
class TestDiscordMemorySystem:
    ...
    async def test_memory_system_initialize_success(self, memory_system, mock_redis_client, mock_postgres_pool):
        ...
        with patch('redis.asyncio.from_url', return_value=mock_redis_client), \
            ...
            mock_context.__aenter__ = AsyncMock(return_value=mock_conn)
            mock_context.__aexit__ = AsyncMock(return_value=None)
            mock_postgres_pool.acquire.return_value = mock_context
