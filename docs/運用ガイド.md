...
# test: 10秒間隔, 100%確率（開発・検証用、リアルトークン/API使用）
# production: 300秒間隔, 33%確率（本番運用、リアルトークン/API使用）
```
...
**重要**: 両環境ともリアルなDiscordトークンとGemini APIキーを使用します。testとproductionの違いは頻度と確率のみです。
...
### **LLM統合エージェント選択**
- **技術基盤**: Gemini 2.0 Flash による文脈考慮エージェント選択
